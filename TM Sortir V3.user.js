...
// @version      74.1.0
// @description  v74.1.0 - BASIS v74.0.0, MODE NONSTOP. ENAM HAL YANG BERUBAH: (1) SATPAM PERMANEN — setInterval 15s yang jalan selamanya, tab FB kebuka = auto start, gak perlu dipencet, gak peduli habis refresh/crash/RDP restart. (2) tryResume warisan v73 DIBUANG — dia masih nyebut akunFb & RUNTIME_CONFIG.komentar yang udah gak ada di v74, jadi ReferenceError diam-diam tiap habis reload. (3) AUTO_RESUME_KEY dibuang total sebagai izin jalan, diganti pagar lagiPindahHalamanSampai (30s) biar satpam gak nyalain loop pas halaman lagi pindah. (4) WATCHDOG DINYALAIN BALIK, 2 tahap: beku 180s → paksa shouldStop (finally jalan → satpam nyalain lagi); masih nyangkut 60s sesudah dipaksa → reload halaman. (5) Tombol STOP + stopMainLoop DIHAPUS, sisa PAUSE. (6) CTA_BLOCK_KEYWORDS tambah 'whatsapp'. Sisanya — CTA anchor, extractUrl, cekKomenKita, closeCommentModal, normalizer — SAMA PERSIS v74.0.0.
// @author       Riko
// @match        *://*.facebook.com/*
// @match        *://*.messenger.com/*
...
(function() {
    ...
    // ============================================================
    // v74.1.0: PAGAR PINDAH HALAMAN
    // navigateToFeedRecover & ensureOnFeedPage nyetel window.location.
    // Halaman butuh waktu buat bener-bener pindah. Selama itu satpam
    // jangan nyalain mainLoop — nanti bentrok sama navigasi.
    // Pagarnya PAKAI WAKTU, bukan flag on/off — kalau navigasinya gagal
    // total, pagar buka sendiri sesudah 30 detik, bot gak mati kunci.
    // ============================================================
    let lagiPindahHalamanSampai = 0;
    const PAGAR_PINDAH_MS = 30000;
    function pasangPagarPindah() { lagiPindahHalamanSampai = Date.now() + PAGAR_PINDAH_MS; }
    function lagiPindahHalaman() { return Date.now() < lagiPindahHalamanSampai; }
    ...
    // v74.1.0: dipakai watchdog tahap 2 — nandain kapan loop udah dipaksa
    // berhenti. Kalau lewat batas masih nyala juga, berarti beneran beku.
    let watchdogForcedStopAt = 0;
    const WATCHDOG_FORCE_RELOAD_MS = 60000;
    ...
    // ============================================================
    // v74.1.0: WATCHDOG DINYALAIN BALIK — 2 TAHAP
    //
    // Dulu dia cuma nulis "WOULD REFRESH ... (disabled)". Artinya kalau
    // loop-nya BEKU (bukan crash — cuma diem nyangkut), mainLoopRunning
    // tetep true, satpam nganggep masih sehat, dibiarin selamanya.
    //
    // Sekarang:
    //   TAHAP 1 — gak ada gerakan 180 detik → shouldStop = true.
    //             Loop keluar baik-baik, blok finally jalan,
    //             mainLoopRunning jadi false → satpam nyalain lagi.
    //             Ini yang paling sering nolong, tanpa reload sama sekali.
    //
    //   TAHAP 2 — udah dipaksa berhenti tapi 60 detik kemudian loop MASIH
    //             nyala = await-nya beneran gak balik-balik. Cuma reload
    //             halaman yang bisa nolong.
    //
    // Catatan: addLog manggil markActivity(), jadi tahap 2 SENGAJA gak
    // ngandelin lastActivityTimestamp — dia baca watchdogForcedStopAt
    // yang gak pernah kesenggol log.
    // ============================================================
    function startWatchdog() {
        ...
        watchdogIntervalId = setInterval(() => {
            try {
                ...
                if (isPaused) { markActivity(); return; }

                if (watchdogForcedStopAt) {
                    if (Date.now() - watchdogForcedStopAt > WATCHDOG_FORCE_RELOAD_MS) {
                        console.warn('[FB Inbox Scraper] Watchdog TAHAP 2: loop beku total, RELOAD halaman');
                        addLog('Watchdog: masih nyangkut sesudah dipaksa berhenti — RELOAD halaman', 'error');
                ...
                if (elapsed > WATCHDOG_TIMEOUT_MS) {
                    watchdogForcedStopAt = Date.now();
                    shouldStop = true;
                    addLog('Watchdog: gak ada gerakan ' + Math.floor(elapsed/1000) + 's — paksa loop berhenti, satpam nyalain lagi', 'error');
                }
            } catch (e) {}
    ...
    // ============================================================
    // v74.1.0: SATPAM PERMANEN
    //
    // Dulu resume-nya rantai setTimeout yang cuma nyambung SEKALI:
    //   setTimeout(() => { if (!mainLoopRunning) tryResume(); }, 5000)
    // Kalau pas 5 detik itu bot lagi jalan → rantai putus, gak ada yang
    // jaga lagi. Loop crash 2 jam kemudian, gak ada yang nyalain.
    //
    // Sekarang setInterval yang jalan SELAMANYA. Tiap 15 detik cek:
    // config keload? keyword ada? bot mati? → nyalain.
    //
    // Gak ada izin, gak ada flag, gak ada tombol. Tab FB kebuka = jalan.
    // Ini yang bikin bot nyambung lagi habis refresh / crash / RDP restart.
    // ============================================================
    const SATPAM_INTERVAL_MS = 15000;
    const SATPAM_FIRST_DELAY_MS = 8000;
    let satpamIntervalId = null;

    function satpamCek() {
        try {
            if (mainLoopRunning) return;
            if (lagiPindahHalaman()) return;
            if (!RUNTIME_CONFIG.loaded) return;
            if (getSkipKeywords().length === 0) return;
            addLog('Satpam: bot lagi mati — nyalain lagi', 'success');
            mainLoop();
        } catch (e) {
            try { console.warn('[FB Inbox Scraper] Satpam error: ' + e.message); } catch (e2) {}
        }
    }

    function startSatpam() {
        if (satpamIntervalId) return;
        addLog('Satpam: aktif — bot bakal dinyalain sendiri terus', 'success');
        setTimeout(satpamCek, SATPAM_FIRST_DELAY_MS);
        satpamIntervalId = setInterval(satpamCek, SATPAM_INTERVAL_MS);
    }
    ...
    //
    // v74.1.0: TAMBAH 'whatsapp'. CTA yang ngarahin ke WA sama aja kayak
    // click-to-message — bukan postingan yang bisa digarap komentar.
    // CATATAN: ini pakai includes(), jadi cocok SEBAGIAN. 'whatsapp' nangkep
    // "Chat WhatsApp", "WhatsApp Sekarang", dst. Tapi CTA yang cuma nulis
    // "WA" doang GAK kena — kalau nanti ketemu varian itu, tambah di sini.
    // ============================================================
    const CTA_BLOCK_KEYWORDS = ['kirim', 'send', 'whatsapp'];
    ...
    function findCtaAdMarkers() {
        ...
        for (const el of candidates) {
            ...
            try { const style = window.getComputedStyle(el); if (style.display === 'none' || style.visibility === 'hidden') continue; if (parseFloat(style.opacity) < 0.1) continue; } catch (e) { continue; }
            if (isInsideComplementary(el)) continue;
            // v73.7.0 + v74.1.0: CTA "Kirim Pesan"/"Send Message"/"WhatsApp" gak dianggap iklan
    ...
    async function mainLoop() {
        ...
        watchdogForcedStopAt = 0;   // v74.1.0: reset tanda "udah dipaksa berhenti"
        ...
        } catch (e) { addLog('Main: loop exception: ' + e.message + ' > tutup modal, lanjut', 'error'); errorRecoveryCount++; try { if (isDialogOpen()) await closeDialogForce(); } catch (err) {} addLog('Main: satpam bakal nyalain lagi ~' + (SATPAM_INTERVAL_MS/1000) + 's lagi', 'info');
        } finally { stopWatchdog(); if (isDialogOpen()) await closeDialogForce(); mainLoopRunning = false; isPaused = false; setPhase('idle', 'Stopped'); addLog('Main: berhenti — kirim=' + collectedLinks.length + ' skipKomen=' + skippedDuplicate + ' noCTA=' + skippedNoCTA + ' gagal=' + gagalCount + ' (satpam bakal nyalain lagi)', 'info'); }
    }
    ...
    function createPanel() {
        ...
        + '<div id="fbs-btn-row-pause" style="display:none;"><button class="fbs-btn fbs-btn-pause" id="fbs-btn-pause">PAUSE</button></div>'
        ...
        // ── v74.1.0: AUTO-START ──
        // Blok tryResume lama DIBUANG. Dia masih nyebut akunFb &
        // RUNTIME_CONFIG.komentar — dua-duanya udah gak ada di v74, jadi
        // begitu dipanggil langsung ReferenceError. Dilempar dari dalam
        // setTimeout, gak ketangkep try/catch di luarnya, mati diam-diam
        // tanpa log. Itu sebabnya tiap habis reload bot gak jalan lagi.
        //
        // Diganti satpam permanen: gak ada izin, gak ada flag, gak ada
        // tombol. Tab FB kebuka = jalan, selamanya.
        startSatpam();
    }
    ...
    async function ensureOnFeedPage() { if (isOnFeedPage()) return false; addLog('Nav: bukan di FEED page, redirect...', 'info'); pasangPagarPindah(); try { if (isDialogOpen()) await closeDialogForce(); } catch (e) {} await sleep(500); window.location.href = FEED_URL; return true; }
    ...
    function updateUI() {
        try {
            ...
            const btnRow = document.getElementById('fbs-btn-row-pause');
            ...
            if (modeStatus) {
                ...
                else { modeStatus.textContent = 'FEED idle — satpam jaga'; modeStatus.style.color = '#b0b3b8'; }
            }
            if (btnStart) {
                ...
                else if (mainLoopRunning) { btnStart.style.display = 'none'; if (btnRow) btnRow.style.display = 'block'; }
